...
//handle amount
  ...
  const handleSubmit = e => {
    ...
    if(charge !== "" && amount > 0) {
     ...
     handleAlert({type: 'danger', text:`charge can't be empty value, amount has to be bigger than 0`})
    }
  }
  // clear all items
  const clearItems = () => {
  ...
  return (
    ...
    <h1>
      Total Spending : {" "} <span className="total"> $ {expenses.reduce((acc,curr) => {return (acc += parseInt(curr.amount));}, 0)}
